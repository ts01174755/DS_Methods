...
def rolling_target_encode(df, 
    ...
    def rolling_prob(df_tmp_, 
        ...
        for index_ in df_seq_unique:
            ...
            df_nparray_tmp_posnum = df_nparray_tmp_posnum \
                + df_nparray_tmp_[df_nparray_tmp_[:,1]==1].shape[0]
    ...
    df_rolling = rolling_prob(df, df_seq_col, df_cla_col, col_newname = 'rolling_prob')
        
    #rolling cond prob
    df = df.sort_values([df_tar_col,df_seq_col], ascending=True, na_position='first')
    df_rolling = df_rolling.groupby(df_tar_col)\
        .apply(rolling_prob,df_seq_col,df_cla_col,'rolling_tar_prob')
        
    #rolling target encode
    df_rolling[col_newname] = Target_lambda*df_rolling['rolling_prob'] \
